...
class Mo_SAML2_Assertion
{
    ...
    {
        $this->id = Mo_SAML_Utilities ::generateId();
        $this->issueInstant = Mo_SAML_Utilities ::generateTimestamp();
        $this->issuer = '';
        $this->authnInstant = Mo_SAML_Utilities ::generateTimestamp();
		...
		if($xml->localName === 'EncryptedAssertion'){
            ...
            if(sanitize_text_field($_POST['RelayState'])=="testValidate"){
        ...
        $this->issueInstant = Mo_SAML_Utilities ::xsDateTimeToTimestamp($xml->getAttribute('IssueInstant'));

        $issuer = Mo_SAML_Utilities ::xpQuery($xml, './saml_assertion:Issuer');
        if (empty($issuer)) {
    ...
    {
        $subject = Mo_SAML_Utilities ::xpQuery($xml, './saml_assertion:Subject');
        if (empty($subject)) {
            /* No Subject node. */
        ...
        $nameId = Mo_SAML_Utilities ::xpQuery(
            $subject,
        ...
        if ($nameId->localName === 'EncryptedData') {
            ...
            $this->nameId = Mo_SAML_Utilities ::parseNameId($nameId);
        }
    }
    ...
    {
        $conditions = Mo_SAML_Utilities ::xpQuery($xml, './saml_assertion:Conditions');
        if (empty($conditions)) {
        ...
        if ($conditions->hasAttribute('NotBefore')) {
            $notBefore = Mo_SAML_Utilities ::xsDateTimeToTimestamp($conditions->getAttribute('NotBefore'));
            if ($this->notBefore === NULL || $this->notBefore < $notBefore) {
                $this->notBefore = $notBefore;
            }
        }
        if ($conditions->hasAttribute('NotOnOrAfter')) {
            $notOnOrAfter = Mo_SAML_Utilities ::xsDateTimeToTimestamp($conditions->getAttribute('NotOnOrAfter'));
        ...
        for ($node = $conditions->firstChild; $node !== NULL; $node = $node->nextSibling) {
            ...
            switch ($node->localName) {
                case 'AudienceRestriction':
                    $audiences = Mo_SAML_Utilities ::extractStrings($node, 'urn:oasis:names:tc:SAML:2.0:assertion', 'Audience');
    ...
    {
        $authnStatements = Mo_SAML_Utilities ::xpQuery($xml, './saml_assertion:AuthnStatement');
        ...
        $this->authnInstant = Mo_SAML_Utilities ::xsDateTimeToTimestamp($authnStatement->getAttribute('AuthnInstant'));

        if ($authnStatement->hasAttribute('SessionNotOnOrAfter')) {
            $this->sessionNotOnOrAfter = Mo_SAML_Utilities ::xsDateTimeToTimestamp($authnStatement->getAttribute('SessionNotOnOrAfter'));
        }
    ...
    private function parseAuthnContext(DOMElement $authnStatementEl)
    {
        ...
        $authnContexts = Mo_SAML_Utilities ::xpQuery($authnStatementEl, './saml_assertion:AuthnContext');
        ...
        $authnContextDeclRefs = Mo_SAML_Utilities ::xpQuery($authnContextEl, './saml_assertion:AuthnContextDeclRef');
        ...
        $authnContextDecls = Mo_SAML_Utilities ::xpQuery($authnContextEl, './saml_assertion:AuthnContextDecl');
        ...
        $authnContextClassRefs = Mo_SAML_Utilities ::xpQuery($authnContextEl, './saml_assertion:AuthnContextClassRef');
        ...
        $this->AuthenticatingAuthority = Mo_SAML_Utilities ::extractStrings(
            $authnContextEl,
    ...
    {
        $firstAttribute = TRUE;
        $attributes = Mo_SAML_Utilities ::xpQuery($xml, './saml_assertion:AttributeStatement/saml_assertion:Attribute');
        foreach ($attributes as $attribute) {
            ...
            $values = Mo_SAML_Utilities ::xpQuery($attribute, './saml_assertion:AttributeValue');
    ...
    {
        $this->encryptedAttribute = Mo_SAML_Utilities ::xpQuery(
            $xml,
    ...
    private function parseSignature(DOMElement $xml)
    {
        ...
        $sig = Mo_SAML_Utilities ::validateElement($xml);
        if ($sig !== FALSE) {
    ...
    public function validate(XMLSecurityKey $key)
    {
        ...
        Mo_SAML_Utilities ::validateSignature($this->signatureData, $key);
    ...
    public function encryptNameId(XMLSecurityKey $key)
    {
        ...
        Mo_SAML_Utilities ::addNameId($root, $this->nameId);
        $nameId = $root->firstChild;

        Mo_SAML_Utilities ::getContainer()->debugMessage($nameId, 'encrypt');
    ...
    public function decryptNameId(XMLSecurityKey $key, array $blacklist = array())
    {
        ...
        $nameId = Mo_SAML_Utilities ::decryptElement($this->encryptedNameId, $key, $blacklist);
        Mo_SAML_Utilities ::getContainer()->debugMessage($nameId, 'decrypt');
        $this->nameId = Mo_SAML_Utilities ::parseNameId($nameId);
    ...
    public function decryptAttributes(XMLSecurityKey $key, array $blacklist = array())
    {
        ...
        foreach ($attributes as $attributeEnc) {
            $attribute = Mo_SAML_Utilities ::decryptElement(
    ...
    public function toXML(DOMNode $parentElement = NULL)
    {
        ...
        if ($this->signatureKey !== NULL) {
            Mo_SAML_Utilities ::insertSignature($this->signatureKey, $this->certificates, $root, $issuer->nextSibling);
        }
    ...
    private function addSubject(DOMElement $root)
    {
        ...
        if ($this->encryptedNameId === NULL) {
            Mo_SAML_Utilities ::addNameId($subject, $this->nameId);
        } else {
    ...
    private function addConditions(DOMElement $root)
    {
        ...
        if ($this->validAudiences !== NULL) {
            ...
            Mo_SAML_Utilities ::addStrings($ar, 'urn:oasis:names:tc:SAML:2.0:assertion', 'saml:Audience', FALSE, $this->validAudiences);
        }
    }
