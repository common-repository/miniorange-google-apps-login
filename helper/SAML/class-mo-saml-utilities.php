...
class Mo_SAML_Utilities  {
	...
	public static function createAuthnRequest( $acsUrl, $issuer,$destination, $force_authn = 'false' ) {
		...
		$requestXmlStr    .= ' ProtocolBinding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST" AssertionConsumerServiceURL="' . $acsUrl .
		                     '" Destination="' . $destination . '"><saml:Issuer xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion">' . $issuer . '</saml:Issuer></samlp:AuthnRequest>';
	...
	public static function createSAMLRequest( $acsUrl, $issuer, $destination, $force_authn = 'false' ) {
		...
		$requestXmlStr .= ' ProtocolBinding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST" AssertionConsumerServiceURL="' . $acsUrl .
		                  '" Destination="' . $destination . '"><saml:Issuer xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion">' . $issuer . '</saml:Issuer><samlp:NameIDPolicy AllowCreate="true" Format="urn:oasis:names:tc:SAML:1.1:nameid-format:unspecified"
                        /></samlp:AuthnRequest>';
	...
	public static function processResponse(
		$currentURL, $certFingerprint, $signatureData,
	...
	public static function metadata_builder( $siteUrl ) {
		$xml = new DOMDocument();
		$url = plugins_url( 'sp-metadata.xml', dirname(dirname(dirname(__FILE__ ))));
		...
		//re-save
		$xml->save( plugins_url( 'sp-metadata.xml', dirname(dirname(dirname(__FILE__ )))) );
	}
