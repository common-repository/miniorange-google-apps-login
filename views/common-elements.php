...
function is_gsuite_customer_registered() {
	...
	echo '<div style="display:block;margin-top:10px;color:red;background-color:rgba(251, 232, 0, 0.15);
		 ...
		 <a href="'.esc_url_raw($registration_url).'">'.mo_gsuite_( "Register or Login with miniOrange") .'</a> 
...
function mo_gsuite_get_country_code_dropdown() {
	...
	foreach ( Mo_Gsuite_CountryList::getCountryCodeList() as $key => $country ) {
		echo '<option data-countrycode="' . esc_html($country['countryCode']) . '" value="' . esc_html($key) . '"';
		echo Mo_Gsuite_CountryList::isCountrySelected( $country['countryCode'], $country['alphacode'] ) ? 'selected' : '';
		echo '>' . esc_html($country['name']) . '</option>';
	}
	echo '</select>';
}
...
function add_plugin_switch( $toggleSwitchValue ) {
	...
	echo '<script>
						jQuery(document).ready(function(){	
							...
							jQuery.ajax({
								url:"' . esc_url_raw(site_url()) . '/?option=miniorange-toggle-oauth-saml",
								type:"POST",
								data:{
								    chekbox_value:e
								},
								...
								dataType:"json",
								success:function(e){
									var current_page= "'.sanitize_text_field($_REQUEST['page']).'";
									if(current_page=="gsuitepricing"){
								        var redirect_url = "' . esc_url_raw(site_url()) . '"+"/wp-admin/admin.php?page=gsuitepricing";
									}else{
								        var redirect_url = "' . esc_url_raw(site_url()) . '"+"/wp-admin/admin.php?page=mogalsettings";
									}
	...
	echo '	<label style="color: Black;">SAML</label>
			<label class="mo-switch">
                <input type="checkbox" id="myCheckbox" value="1"' . esc_html($toggleSwitchValue) . '>
...
function mo_gsuite_handle_oauth_saml_switch() {
	...
	if ( strcasecmp( trim( $_REQUEST['option'] ), 'miniorange-toggle-oauth-saml' ) == 0 ) {
		$checkBoxValue = sanitize_text_field($_REQUEST["chekbox_value"]);
...
function mo_saml_get_test_url() {
	if ( get_option( 'mo_saml_enable_cloud_broker' ) == 'false' || get_option( 'mo_saml_enable_cloud_broker' ) == 'miniorange' ){
		$url = esc_url_raw(site_url()) . '/?option=testConfig';
	} else {
...
function miniorange_generate_metadata() {
	...
	if ( empty( $sp_entity_id ) ) {
		$sp_entity_id = plugins_url('/', dirname(dirname(__FILE__)));
	}
...
<md:EntityDescriptor xmlns:md="urn:oasis:names:tc:SAML:2.0:metadata" validUntil="2020-10-28T23:59:59Z" cacheDuration="PT1446808792S" entityID="' . esc_html($entity_id) . '">
  <md:SPSSODescriptor AuthnRequestsSigned="false" WantAssertionsSigned="true" protocolSupportEnumeration="urn:oasis:names:tc:SAML:2.0:protocol">
    <md:NameIDFormat>urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress</md:NameIDFormat>
    <md:AssertionConsumerService Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST" Location="' . esc_url_raw( $acs_url ) . '" index="1"/>
  </md:SPSSODescriptor>
  <md:Organization>
