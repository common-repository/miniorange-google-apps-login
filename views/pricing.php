...
echo ' <div class="mo_gsuite_registration_pricing_layout" style="width:' . esc_html($width) . '">';
...
if ( ! $vl ) {
	echo '<input type="button" ' . esc_html($disabled) . ' name="check_btn" id="check_btn" 
...
echo '			<input type="button" name="ok_btn" id="ok_btn" class="button button-primary button-large" 
						value="' . mo_gsuite_( "OK, Got It" ) . '" onclick="window.location.href=\'' . esc_url_raw($settings) . '\'" />
			</span>
		<h2>
		<hr>
...
if ( ! $plan ) {
	echo '			<td>
					<div class="mo_registration_thumbnail mo_registration_pricing_free_tab" >
						<h3 class="mo_registration_pricing_header">' . esc_html($free_plan_name) . '</h3>
...
echo esc_html($basic_plan_name) . '
					</h3>
						<h4 class="mo_registration_pricing_sub_header">';
if ( $vl ) {
	echo '							<input type="button" style="margin-bottom:3.8%;" ' . esc_html($disabled) . ' 
...
echo '				<hr>
					<p style="margin-bottom: 17.7%;" class="mo_registration_pricing_text"><b>' . esc_html($basic_plan_price) . '</b><br/><br/>
...
echo esc_html($premium_plan_name) . '
					</h3>
				<h4 class="mo_registration_pricing_sub_header">';
if ( strcmp( $plan, Mo_Gsuite_Constants::PCODE ) != 0 && strcmp( $plan, Mo_Gsuite_Constants::BCODE ) != 0 && strcmp( $plan, Mo_Gsuite_Constants::CCODE ) != 0 ) {
	echo '							<input type="button" style="margin-bottom:3.8%;"  ' . esc_html($disabled) . ' class="button button-primary button-large" 
...
echo ' 			</h4>
				<hr>
				<div style="margin-top:4.5%;" class="mo_registration_pricing_text"><b>' . esc_html($premium_plan_price) . '</b><br/>+</div>
					<table>
					<tr>
...
echo '
		...
		<div id="disclaimer" style="margin-bottom:15px;">
			<span style="font-size:15px;">
				...
				*' . mo_gsuite_( "Transaction prices may very depending on country. If you want to use more than 50k transactions, mail us at" ) . ' 
					...
					<a href="' . esc_url_raw($profile_url) . '">' . mo_gsuite_( "Profile tab" ) . '</a>.<br/><br/>
				**' . mo_gsuite_( "If you want to <b>use miniorange SMS/SMTP gateway</b>, and your country is not in list, mail us at" ) . ' <a href="mailto:' . Mo_Gsuite_Constants::SUPPORT_EMAIL . '"><b>' . Mo_Gsuite_Constants::SUPPORT_EMAIL . '</b></a>
						' . mo_gsuite_( "or submit a support request using the support form under User" ) . ' 
						<a href="' . esc_url_raw($profile_url) . '">' . mo_gsuite_( "Profile tab" ) . '</a>. 
	 ...
	 <form style="display:none;" id="mocf_loginform" action="' . esc_url_raw($form_action) . '" target="_blank" method="post">
		<input type="email" name="username" value="' . esc_html($email) . '" />
		<input type="text" name="redirectUrl" value="' . esc_url_raw($redirect_url) . '" />
